// Test foundation verification, run with: npm run verify:test-foundation
//
...
//   3. npm run typecheck — tsc --noEmit over the whole repo incl. tests
//
...
if (!hasDeps) {
  ...
  if (status !== 0) {
    console.error("\nverify:test-foundation FAILED at dependency install");
    process.exit(status);
  }
}
...
if (status !== 0) {
  console.error("\nverify:test-foundation FAILED at vitest run");
  process.exit(status);
}

status = run("npm", ["run", "typecheck"]);
if (status !== 0) {
  console.error("\nverify:test-foundation FAILED at typecheck");
  process.exit(status);
}

console.log("\nverify:test-foundation PASSED (vitest run + typecheck)");
